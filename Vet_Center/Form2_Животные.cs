...
{
    public partial class Form2_Животные : Form
    {
        public Form2_Животные()
        {
            InitializeComponent();
        }
        ...
        private void Животные(object sender, EventArgs e)
        {
        ...
        {
            SqlDataAdapter sqlDataAdapter = new SqlDataAdapter("Select * From Животные", sqlConnection);
            DataSet dataset = new DataSet();
            sqlDataAdapter.Fill(dataset, "Животные");
            dataGridView.DataSource = dataset.Tables["Животные"];
        }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string q = $"INSERT INTO Животные (Код_хозяина, Кличка, Порода) VALUES ('{text1}',N'{text5}',N'{text3}')";
        ...
        private void Удалить_Click(object sender, EventArgs e)
        {
            ...
            string q = $"DELETE FROM Животные WHERE id = '{textBox6.Text}'";
            SqlCommand command = new SqlCommand(q, sqlConnection);
        ...
        {
            (dataGridView1.DataSource as DataTable).DefaultView.RowFilter = $"Наименование like '%{textBox7.Text}%'OR Назначение like '%{textBox7.Text}%'";
        }
        ...
        private void Изменить_Click(object sender, EventArgs e)
        {
            ...
            if (Column == "Birthday")
            {
                q = $"UPDATE Животные set {Column} = '{date.ToString("yyyy-MM-dd")}' WHERE id = {Id}";
            }
            else { q = $"UPDATE Животные set {Column} = N'{textBox2.Text}' WHERE id = {Id}"; }
            SqlCommand command = new SqlCommand(q, sqlConnection);
            command.ExecuteNonQuery();
            GetList(dataGridView1);
        }
